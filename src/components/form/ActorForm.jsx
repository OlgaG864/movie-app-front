...
export default function ActorForm({ title, btnTitle, busy, onSubmit }) {
  ...
  return (
    <form
      className="dark:bg-main bg-white p-3 w-[35rem] rounded"
      onSubmit={handleSubmit}
    >
      <div className="flex justify-between items-center mb-3">
        <h1 className="font-semibold text-xl dark:text-white text-main">
          {title}
        </h1>
        <button
          className="h-8 w-24 bg-main text-white dark:bg-white dark:text-main hover:opacity-80 transition rounded flex items-center justify-center"
          type="submit"
        >
          {busy ? <i className="bi bi-arrow-clockwise"></i> : btnTitle}
        </button>
      </div>
